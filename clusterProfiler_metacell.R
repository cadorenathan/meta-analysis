setwd("C:/Users/cador/OneDrive/Doutorado/Meta SBGM/Meta_cell_SVA")
getwd()

library(dplyr)

genes <- read.csv("meta_result_cell.csv", header=T)
genes <- filter(genes, metap <= 0.05, metafc <= -1 | metafc >= 1)
...
genes.map <-select(org.Hs.eg.db, as.character(genes$SYMBOL),c("SYMBOL","ENTREZID"), "SYMBOL")
colnames(genes)[1] <- "SYMBOL"
genes.map <- merge(genes, genes.map)

setwd("C:/Users/cador/OneDrive/Doutorado/Meta SBGM/clusterProfileR/Utilizando apenas significativos")
getwd()

genes.map <- read.csv("EntrezID_metacell.csv", header=T)
...
enrich.kegg <- enrichKEGG(gene = unique(na.omit(genes.map$ENTREZID)),
                          ...
                          pAdjustMethod = "fdr")
# não consegui fazer o download do pacote KEGG.db então troquei o use_internal_data para FALSE

enrich.kegg <- setReadable(enrich.kegg,org.Hs.eg.db,keyType = "ENTREZID")
kegg.table <- as.data.frame(enrich.kegg)
View(kegg.table)
write.csv(kegg.table, "KEGG_meta_metacell.csv")
...
write.csv(go.table, "GO_targets_metacell.csv")
write.csv(go2.table, "GO2_table_limb_HPO_metacell.csv")

jpeg(file="dotplot_KEGG_metacell.jpeg", width=6200, height=4800, units="px", res=600)
dotplot(enrich.kegg, showCategory=20, orderBy="GeneRatio")
dev.off()

jpeg(file="dotplot_targets_metacell.jpeg", width=6200, height=4800, units="px", res=600)
dotplot(ego, showCategory=20)
dev.off()
enrichMap(ego)

jpeg(file="cnetplot_GO_metacell.jpeg", width=6200, height=4800, units="px", res=600)
...
View(rPA2)
write.csv(rPA2, "Reactome_metacell.csv")

jpeg(file="dotplot_Reactome_metacell.jpeg", width=6200, height=4800, units="px", res=600)
...
jpeg(file="dotplot_DGN_metacell.jpeg", width=8200, height=4800, units="px", res=600)
dotplot(dgn, showCategory=20)
dev.off()

write.csv(dgn.table, "dgn_metacell.csv")
